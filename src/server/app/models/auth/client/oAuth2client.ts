...
import { AuthClient } from '../abstract/authClient';
import { DefaultTransporter, RequestError } from '../abstract/transporters';
...
export default class OAuth2client implements AuthClient {
    ...
    public generateAuthUrl(optOpts: IGenerateAuthUrlOpts) {
        ...
        if (optOpts.codeVerifier !== undefined) {
            ...
            options.code_challenge = OAuth2client.BASE64URLENCODE(
                OAuth2client.SHA256(optOpts.codeVerifier)
            );
        }
    ...
    public async refreshAccessToken(): Promise<ICredentials> {
        ...
        return this.refreshToken(this.credentials.refresh_token).then(
            (tokens) => {
    ...
    public async getAccessToken(): Promise<string> {
        ...
        const shouldRefresh =
            this.credentials.access_token === undefined || isTokenExpired;
        ...
        while (numberOfTry >= 0) {
            try {
                ...
                if (error instanceof Error) {
                    const statusCode = (<RequestError>error).code;
                    ...
                    if (
                        ...
                        /* istanbul ignore else */
                        if (
                            typeof this.credentials.refresh_token ===
                                'string' &&
                            this.credentials.refresh_token.length > 0
                        ) {
                            await this.refreshAccessToken();
    ...
    private verifySignedJwt(jwt: string, requiredAudience?: string | string[]) {
        ...
        try {
            envelope = JSON.parse(
                new Buffer(segments[0], 'base64').toString('utf8')
            );
        } catch (err) {
        ...
        try {
            payload = JSON.parse(
                new Buffer(segments[1], 'base64').toString('utf8')
            );
        } catch (err) {
        ...
        if (payload.iat === undefined) {
            throw new Error(
                `No issue time in token: ${JSON.stringify(payload)}`
            );
        }
